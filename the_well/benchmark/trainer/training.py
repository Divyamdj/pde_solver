...
class Trainer:
    ...
    def rollout_model(self, model, batch, formatter, train=True):
        ...
        if train and (not hasattr(self, "_saved_debug_batch")):
            os.makedirs("debug_batches", exist_ok=True)
            torch.save(
                {"raw_batch": batch, "inputs": inputs, "y_ref": y_ref},
                "debug_batches/formatted_inputs.pt"
            )
            print("Saved formatted model inputs to debug_batches/formatted_inputs.pt")
            self._saved_debug_batch = True
        ...
        for i in range(rollout_steps):
            ...
            # Pass pde_embedding only if available AND model is conditioned (accepts it)
            if "pde_embedding" in batch and "Conditioned" in model.__class__.__name__:
                pde_embedding = batch["pde_embedding"].to(self.device)
                y_pred = model(*inputs, pde_embedding=pde_embedding)
            else:
                y_pred = model(*inputs)
    ...
    def train_one_epoch(self, epoch: int, dataloader: DataLoader) -> float:
        ...
        pbar = tqdm.tqdm(
            ...
            desc=f"Epoch {epoch+1}/{self.max_epoch} [train]",
            leave=True,
        ...
        for i, batch in enumerate(pbar):
            if epoch == 1 and i == 0:
                os.makedirs("debug_batches", exist_ok=True)
                torch.save(batch, "debug_batches/raw_batch_epoch1.pt")
                print("Saved raw batch to debug_batches/raw_batch_epoch1.pt")
            ...
            with torch.autocast(
                ...
                if epoch == 1 and i == 0:
                    # batch["input_fields"] is (B, Ti, H, W, C)
                    x = batch["input_fields"].detach().cpu()
                    y = batch["output_fields"].detach().cpu()

                    # take sample 0
                    x0 = x[0]   # (Ti, H, W, C)
                    y0 = y[0]   # (To, H, W, C)

                    # choose last input timestep and first output timestep
                    x_img = x0[-1]   # (H, W, C)
                    y_img = y0[0]    # (H, W, C)

                    # log each channel as a separate image
                    log_dict = {}
                    for c in range(x_img.shape[-1]):
                        log_dict[f"debug/input_last_t/channel_{c}"] = wandb.Image(x_img[:, :, c].numpy())
                        log_dict[f"debug/output_t0/channel_{c}"] = wandb.Image(y_img[:, :, c].numpy())
