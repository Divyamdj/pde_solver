...
def get_experiment_name(cfg: DictConfig) -> str:
    ...
    # Handle both single-dataset and multi-dataset configs
    if hasattr(cfg.data, "train_datasets"):
        # Multi-dataset mode
        dataset_name = "_".join(cfg.data.train_datasets)
    else:
        # Single-dataset mode
        dataset_name = cfg.data.well_dataset_name
    
    return f"{dataset_name}-{cfg.name}-{model_name}-{cfg.optimizer.lr}"
